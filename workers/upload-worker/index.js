...
// that stops anonymous abuse of R2 from outside the app. Verification is done
// locally against Google's public signing keys — no extra network hop per call
...
// Throws if no valid token is present. `REQUIRE_AUTH=false` env var is an escape
// hatch for local testing only — leave it unset in production.
async function requireUser(request, env) {
  if (env.REQUIRE_AUTH === "false") return { sub: "auth-disabled" };
...
// Note: in-memory maps are per-isolate. For strict global limits use Durable
// Objects (paid). This still stops most abuse — each isolate enforces its window.
const _ipWindows  = new Map(); // ip  → [timestamp, ...]
const _uidWindows = new Map(); // uid → [timestamp, ...]
...
// Returns true if the caller is within limits; false if they should be throttled.
// ipLimit:  max requests per ipWindowMs for this IP
// uidLimit: max requests per uidWindowMs for this UID (pass null to skip)
...
// Verifies that the actual file bytes match the declared MIME type.
// Prevents MIME spoofing (e.g. an .exe renamed to .pdf).
function verifyMagicBytes(buffer, mimeType) {
  ...
  // For other allowed types (xlsx, csv, docx, zip) we do not magic-byte check
  // as they share the PK zip header and content validation happens elsewhere.
  return true;
}
...
async function handleUpload(request, env, origin, workerOrigin) {
  ...
  // Rate limit: 10 uploads/minute per IP, 30 uploads/hour per user
  const ip = request.headers.get("CF-Connecting-IP") || "";
  ...
  try {
    ...
    // Sanitize filename: strip path traversal, control chars, keep only safe chars
...
async function handleLibraryUpload(request, env, origin, workerOrigin) {
  ...
  // Rate limit: 5 library uploads/minute per IP, 20/hour per user
  const ip = request.headers.get("CF-Connecting-IP") || "";
  ...
  try {
    ...
    if (rule.action === "approve") {
      ...
      // Still store the file — a librarian may override, but status is "rejected"
      ...
      moderationStatus =
        gem.action === "approve" ? "approved" :
        gem.action === "reject"  ? "rejected"  : "under_review";
    }
...
async function handleDelete(request, env, origin) {
  try {
    ...
    if (body.prefix) {
      // Mass deletion requires BOTH a valid Firebase ID token AND the admin secret.
      // Stealing the Firestore-stored secret alone is not enough — the attacker
      // also needs an active signed-in session from the admin account.
    ...
    // Per-file deletes (signature swap, librarian removing a file) require a
    // valid signed-in user — never anonymous.
    try { await requireUser(request, env); }
    catch (e) { return json({ error: "Unauthorized — " + e.message }, 401, origin); }
...
async function handleDeleteAuthUser(request, env, origin) {
  // Rate limit admin endpoints: 20 calls/hour per IP
  const ip = request.headers.get("CF-Connecting-IP") || "";
  ...
  try {
    ...
    if (!uid)
      return json({ error: "uid required" }, 400, origin);
...
// ── Admin password reset (used by the Industrial-Training-Secretary card) ──────
...
// Only our own front-ends may call the Worker from a browser. Anything else gets
// an origin it can't match, so the browser blocks the response. (Non-browser
// clients ignore CORS — token verification is what stops those.)
...
function pickOrigin(origin) {
  ...
  return ALLOWED_ORIGINS[0]; // deny: hand back an origin the caller isn't on
}
